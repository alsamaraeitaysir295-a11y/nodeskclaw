...
const FORMAT_SPEC_MD = [
  ...
  '## 命名要求（重要）',
  '',
  '- summary 必须简短（≤20 字），只写功能名称本身',
  '- 不要在 summary 里写补充说明（放入 description 字段）',
  '- 正确：summary: "查询订单"',
  '- 错误：summary: "查询订单 — 支持按状态/日期/产线筛选，与 /api/v2/orders 行为相同"',
  '- platformId 写英文蛇形命名（如 queryOrders、getOrderDetail）',
  '',
  '## 最小示例',
  '',
  '    {',
  ...
  '要求：',
  '- summary 必须简短（≤20 字），只写功能名称，不要写补充说明',
  '- 补充说明放入 description 字段',
  '- 每个参数都写中文 description 和 example',
  '- 响应如果包含完整文本回答，在 200 响应的 schema 里定义 answer 字符串字段',
  '- 不要使用 allOf"',
  '',
  '完整版规范请联系平台管理员获取。',
].join('\n') + '\n'
